...
loc_data = pd.read_csv('templates\sf_50_bbox_info.csv', delimiter="|")
print(loc_data.shape)

OCR_LOCATIONS = [
    OCRLocation("name", (60, 188, 800, 61),
                ["Name", "Last", "First", "Middle"]),
    OCRLocation("ssn", (864, 188, 292, 61), ["Social", "Security", "Number"])
]
...
for (locID, result) in results.items():
    ...
    for (i, line) in enumerate(text.split("\n")):
        ...
        startY = y + (i * 70) + 40
        cv2.putText(aligned, line, (x, startY), cv2.FONT_HERSHEY_SIMPLEX, 1.8,
                    (0, 0, 255), 5)
